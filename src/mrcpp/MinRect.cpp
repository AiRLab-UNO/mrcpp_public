...
namespace mrcpp::minrect{
    ...
    double MinRect::_distance(const Point &start, const Point &end, const Point &point)
    {
        double dist = (point.y - start.y) * (end.x - start.x) - (point.x - start.x) * (end.y - start.y);
        return dist;
    }

    std::vector<Point> MinRect::convexHull(const std::vector<Point>& input)
    {
        std::vector<Point> P = input;
        size_t n = P.size();
        size_t k = 0;
        if (n <= 3) return P;

        std::vector<Point> H(2 * n);
        std::sort(P.begin(), P.end(), [](const wykobi::point2d<double>& a, const wykobi::point2d<double>& b) {
    ...
    double MinRect::diameter(const std::vector<Point> &p)
    {
        ...
        {
            res = std::max(res, dist(h[i], h[j]));
            while (j < m && area(h[i], h[(i + 1) % m], h[(j + 1) % m]) > area(h[i], h[(i + 1) % m], h[j]))
            {
                res = std::max(res, dist(h[i], h[(j + 1) % m]));
                j++;
            }
        }
        return res;
    ...
    MinAreaRect MinRect::minAreaRect(const std::vector<Point> &p)
    {
        std::vector<Point> points = convexHull(p);
...
//        cout << "(" << points[i].x << "," << points[i].y << ")," << endl;
        ...
        std::vector<Point> vect(n);
        std::vector<double> inv_vect_length(n);
        ...
        Point out0 = wykobi::make_point(px, py);
        Point out1 = wykobi::make_point(A1 * min_area_state.width, B1 * min_area_state.width);
        Point out2 = wykobi::make_point(A2 * min_area_state.height, B2 * min_area_state.height);
        ...
        min_area_rect.rect.push_back(wykobi::make_point(out0.x + out1.x, out0.y + out1.y));
        min_area_rect.rect.push_back(wykobi::make_point(out0.x + out1.x + out2.x, out0.y + out1.y + out2.y));
        min_area_rect.rect.push_back(wykobi::make_point(out0.x + out2.x, out0.y + out2.y));
